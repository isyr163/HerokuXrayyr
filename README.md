# 使用He*ku部署Xr*y高性能代理服务，通过ws传输的 (vme*s、vl*s、tro*an shado*socks、s*cks)等协议

> 提醒： 滥用可能导致账户被BAN！！！ 

> 提醒： He*ku 已经封禁本专案，请 Fork 本专案后，将 README.md 中的 Henryaay 替换为自己的用户名后，再进行部署。 
>![image](https://user-images.githubusercontent.com/5351277/126950598-7930a0ac-739a-46ac-aef2-afa2d213a06c.png)

## 概述

用于在 Her*上部署 vless+websocket+tls，每次部署自动选择最新的 alpine linux 和 Xray core 。  
vless 性能更加优秀，占用资源更少。
...
* 支持存储自定义文件,目录及账号密码均为UUID,客户端务必使用TLS连接  
  **Her*ku 为我们提供了免费的容器服务，我们不应该滥用它，所以本项目不宜做为长期翻墙使用。**
...
[![Deploy](https://www.herokucdn.com/deploy/button.png)](https://dashboard.heroku.com/new?template=https://github.com/isyr163/yrku) 
...
* **务必替换所有的`6efbe530-0f62-4fab-90f5-dcb5cb6b4855`为部署时设置的UUID,建议更改,不要每个人都一样 （app.josn里改默认）**  
